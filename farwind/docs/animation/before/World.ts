...
export class World extends Phaser.Scene {
  ...
  async start(continued: boolean) {
    ...
    this.invulnerable = 0;
    this.history = [];
    this.cat.move(this.state.player.x + 40, this.state.player.y, 0, 0, 0);
    if (this.blocked(this.cat.sprite.x, this.cat.sprite.y))
      this.cat.move(this.state.player.x, this.state.player.y, 0, 0, 0);
    this.hero.move(this.state.player.x, this.state.player.y, 0, 0, 0);
  ...
  update(_time: number, delta: number) {
    ...
    this.hero.move(p.x, p.y, dx, dy, this.sim, this.sim < this.attackUntil);
    ...
    if (length) {
      ...
      if (!last || Math.hypot(p.x - last.x, p.y - last.y) > 12)
        this.history.push({ x: p.x, y: p.y });
      if (this.history.length > 250) this.history.shift();
    }
    const cat = this.cat.sprite;
    while (
      this.history.length > 1 &&
      Math.hypot(cat.x - this.history[0].x, cat.y - this.history[0].y) < 15
    )
      this.history.shift();
    const dest = this.history[0];
    if (dest && Math.hypot(cat.x - p.x, cat.y - p.y) > 48) {
      const cx = dest.x - cat.x,
        cy = dest.y - cat.y,
        l = Math.hypot(cx, cy),
        step = Math.min(l, 240 * dt);
      if (l && this.clearLine(cat.x, cat.y, dest.x, dest.y))
        this.cat.move(
          cat.x + (cx / l) * step,
          cat.y + (cy / l) * step,
          cx,
    ...
    } else this.cat.move(cat.x, cat.y, 0, 0, this.sim);
    this.target = props
      .filter(
        (o) =>
          o.kind &&
    ...
    if (p.hp <= 0) {
      ...
      p.y = 720;
      this.history = [];
      this.cat.move(720, 740, 0, 0, this.sim);
