...
const baseline = new CombatController();
const stages: number[] = [];
const tick = (a:number,b:number) => baseline.update(a,b,3,{x:0,y:0},[],()=>false,()=>true,()=>{},stage=>stages.push(stage));
baseline.requestAttack(0);tick(0,0);baseline.requestAttack(60);tick(0,300);
console.log(JSON.stringify({说明:"旧150ms规则的60ms早按对照，不是跨事件修复复发",段序列:stages,仍有预约:baseline.pending}));
